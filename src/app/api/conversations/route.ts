  ...
  try {
    ...
    if (!backendRes.ok) {
      ...
      if (backendRes.status >= 500) {
        errorMessage = 'An unexpected server error occurred while fetching conversations. Please try again later.'; 
      } 
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!backendRes.ok) {
      ...
      if (backendRes.status >= 500) {
        errorMessage = 'An unexpected server error occurred while deleting the conversation. Please try again later.'; 
      }
