...
import { submitPublicReport, sendChatMessage, PublicReportSubmission, Message } from '@/lib/public_reports'; 
...
function ReportScam() {
  ...
  const validationSchema = yup.object({
    ...
    contact_no: yup.string().required("Contact number is required").matches(/^\+?\d{8,}$/, { message: "Contact number must have at least 8 digits (optional + prefix)" }),
    email: yup.string().email("Invalid email address").required("Email is required"),
    sex: yup.string().required("Sex is required"),
    dob: yup.string().nullable()
    .matches(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format (YYYY-MM-DD)", excludeEmptyString: true })
    .test('is-valid-date', "Invalid date", value => !value || dayjs(value, "YYYY-MM-DD", true).isValid())
    .test('not-future', "Date of birth cannot be in the future", value => !value || !dayjs(value).isAfter(dayjs(), 'day')),
    ...
    role: yup.string().oneOf(["reportee", "victim", "witness"], "Invalid role").required("Role is required"),
    scam_incident_date: yup.string().required("Incident date is required")
    .matches(/^\d{4}-\d{2}-\d{2}$/, "Invalid date format (YYYY-MM-DD)")
    .test('is-valid-date', "Invalid date", value => dayjs(value, "YYYY-MM-DD", true).isValid())
    .test('not-future', "Incident date cannot be in the future", value => !dayjs(value).isAfter(dayjs(), 'day')),
    ...
    scam_amount_lost: yup.number().nullable().min(0, "Amount lost cannot be negative"),
    scam_incident_description: yup.string().trim().required("Description is required").min(1, "Description cannot be empty"),
  });

  const { control, handleSubmit, reset, watch, formState: { errors }, setValue: setFormField } = useForm<PublicReportSubmission>({
  ...
  const [otherCommunicationPlatform, setOtherCommunicationPlatform] = useState("");
  ...
  const [aiMessages, setAiMessages] = useState<Message[]>([
    {
      role: "ai",
      content: "Hello, I’m here to assist you. Could you please describe your experience with the scam? (It may take a short while for me to process the details, but I’ll guide you through and fill the form based on your inputs.)",
    },
  ]);
  ...
  const [submittedData, setSubmittedData] = useState<PublicReportSubmission | null>(null);
  ...
  const fuzzyMatch = (input: string, options: string[]): string | null => {
    ...
    options.forEach(option => {
      ...
      const score = Math.max(
        ...
        (normalizedInput.split('').filter(char => normOption.includes(char)).length / Math.max(normalizedInput.length, normOption.length)) * 100
      );
  ...
  const applyAiOverrides = (suggestions: Record<string, any>) => {
    const fieldsNotToUpper = ['scam_email', 'scam_url_link', 'scam_moniker','scam_incident_description'];
    Object.entries(suggestions).forEach(([key, value]) => {
      if (value && key.startsWith('scam_')) {
        ...
        if (['scam_type', 'scam_approach_platform', 'scam_communication_platform', 'scam_transaction_type'].includes(key)) {
          const options = {
            scam_type: ['ECOMMERCE', 'PHISHING', 'GOVERNMENT OFFICIALS IMPERSONATION'],
            scam_approach_platform: ['CALL', 'SMS', 'EMAIL', 'WHATSAPP', 'TELEGRAM', 'FACEBOOK', 'INSTAGRAM', 'OTHERS', 'WEBSITE'],
            scam_communication_platform: ['CALL', 'SMS', 'EMAIL', 'WHATSAPP', 'TELEGRAM', 'FACEBOOK', 'INSTAGRAM', 'OTHERS', 'WEBSITE'],
            scam_transaction_type: ['BANK TRANSFER', 'CREDIT CARD', 'CRYPTOCURRENCY', 'CASH', 'E-WALLET', 'OTHERS'],
          }[key] || [];
          ...
          if (match) {
            ...
            console.log(`No match for ${key}, setting to 'OTHERS' with other: ${finalValue}`); // Log fallback
  ...
  const onSubmit = async (data: PublicReportSubmission) => {
    ...
    const othersChecks = [
      ...
      { field: data.scam_approach_platform, other: otherApproachPlatform, name: "approach platform" },
      { field: data.scam_communication_platform, other: otherCommunicationPlatform, name: "communication platform" },
      { field: data.scam_transaction_type, other: otherTransactionType, name: "transaction type" },
    ];
    ...
    data.scam_type = data.scam_type === "OTHERS" ? otherScamType : data.scam_type;
    data.scam_approach_platform = data.scam_approach_platform === "OTHERS" ? otherApproachPlatform : data.scam_approach_platform;
    data.scam_communication_platform = data.scam_communication_platform === "OTHERS" ? otherCommunicationPlatform : data.scam_communication_platform;
    data.scam_transaction_type = data.scam_transaction_type === "OTHERS" ? otherTransactionType : data.scam_transaction_type;

    // Uppercase fields as per public_reports.py
    const personFieldsToUpper = ['first_name', 'last_name', 'sex', 'nationality', 'race', 'occupation', 'blk', 'street', 'unit_no', 'postcode'];
    personFieldsToUpper.forEach(field => {
      if (data[field]) data[field] = data[field].toUpperCase();
    });
    const reportFieldsToUpper = ['scam_type', 'scam_approach_platform', 'scam_communication_platform', 'scam_transaction_type', 'scam_beneficiary_platform', 'scam_beneficiary_identifier'];
    ...
    if (data.scam_incident_date && typeof data.scam_incident_date !== 'string') {
      data.scam_incident_date = dayjs(data.scam_incident_date).format("YYYY-MM-DD");
    }
    ...
    try {
      ...
      setSnackbarMessage(err.message || "An error occurred while submitting the report.");
      setSnackbarOpen(true);
  ...
  const handleDownloadReport = () => {
    if (!submittedData) return;  // Safety check if data not captured
  
    const doc = new jsPDF();
    const pageHeight = 280;  // Safe content height per page (A4 is ~297mm, leave margins)
    let yPos = 20;  // Starting Y, top margin
    
    // Helper function to add text with auto-pagination
    const addText = (text: string | string[], fontSize: number, isBold: boolean = false, x: number = 20) => {
      ...
      yPos += requiredHeight + (isBold ? 5 : 2);  // Extra spacing after headers
    };

    const addDivider = () => {
      if (yPos + 5 > pageHeight) {  // Check for divider too
        doc.addPage();
        yPos = 20;
      }
      doc.setLineWidth(0.5);
      doc.line(20, yPos, 190, yPos);
      yPos += 10;  // Space after divider
    };

    // Title: Centered, large, bold
    doc.setTextColor(0, 0, 128);  // Dark blue for title
    addText("Scam Report Confirmation", 18, true, 105);  // Centered via manual x=105 or use {align: 'center'} if preferred
    doc.setTextColor(0, 0, 0);  // Reset to black

    // Subtitle: Report ID and Date
    ...
    doc.text("Victim Details", 20, yPos);
    yPos += 10;
    doc.setFontSize(11);
    doc.setFont("helvetica", "normal");
    doc.text(`Name: ${submittedData.first_name} ${submittedData.last_name}`, 20, yPos);
    yPos += 7;
    doc.text(`Sex: ${submittedData.sex}${submittedData.sex === "Others" ? ` (${otherSex})` : ""}`, 20, yPos);
    yPos += 7;
    doc.text(`Date of Birth: ${submittedData.dob || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Nationality: ${submittedData.nationality || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Race: ${submittedData.race ? submittedData.race + (submittedData.race === "Others" ? ` (${otherRace})` : "") : "N/A"}`, 20, yPos);
    yPos += 7;
    ...
    yPos += 7;
    doc.text(`Address: ${submittedData.blk ? `Blk ${submittedData.blk}, ` : ""}${submittedData.street ? `${submittedData.street}, ` : ""}${submittedData.unit_no ? `Unit ${submittedData.unit_no}, ` : ""}${submittedData.postcode || "N/A"}`, 20, yPos);
    yPos += 10;
    ...
    yPos += 7;
    doc.text(`Scam Type: ${submittedData.scam_type ? submittedData.scam_type + (submittedData.scam_type === "OTHERS" ? ` (${otherScamType})` : "") : "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Amount Lost (SGD): ${submittedData.scam_amount_lost || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Approach Platform: ${submittedData.scam_approach_platform ? submittedData.scam_approach_platform + (submittedData.scam_approach_platform === "OTHERS" ? ` (${otherApproachPlatform})` : "") : "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Communication Platform: ${submittedData.scam_communication_platform ? submittedData.scam_communication_platform + (submittedData.scam_communication_platform === "OTHERS" ? ` (${otherCommunicationPlatform})` : "") : "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Transaction Type: ${submittedData.scam_transaction_type ? submittedData.scam_transaction_type + (submittedData.scam_transaction_type === "OTHERS" ? ` (${otherTransactionType})` : "") : "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Beneficiary Platform: ${submittedData.scam_beneficiary_platform || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Beneficiary Identifier: ${submittedData.scam_beneficiary_identifier || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Scammer Contact No: ${submittedData.scam_contact_no || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Scammer Email: ${submittedData.scam_email || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Scammer Moniker: ${submittedData.scam_moniker || "N/A"}`, 20, yPos);
    yPos += 7;
    doc.text(`Scam URL/Link: ${submittedData.scam_url_link || "N/A"}`, 20, yPos);
    yPos += 10;
    ...
    const descriptionLines = doc.splitTextToSize(submittedData.scam_incident_description || "N/A", 170);
  ...
  const handleAiSend = async () => {
    ...
    const formDataSummary = Object.entries(watch()).filter(([key, val]) => val && key.startsWith('scam_')) // Only scam fields
      .map(([key, val]) => `${key}: ${val}`).join(', ');
    const enhancedQuery = formDataSummary ? `Current form data: ${formDataSummary}. User query: ${aiInput}` : aiInput;
    ...
    try {
      const { response, conversation_id, structured_data } = await sendChatMessage(enhancedQuery, conversationId);
        ...
        >
          ...
          >
            ...
            <Typography variant="body2" sx={{ mb: 1 }}>
              Use these narratives to test the chatbot (copy-paste directly into the AI chat). They align with trained scam types (Ecommerce, Phishing, Government Impersonation). Other inputs may produce suboptimal results.
            </Typography>
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, sm: 3 }}>
              <FormControl fullWidth sx={{ minWidth: 200 }}>
                ...
                <FormHelperText error={!!errors.race}>{errors.race?.message}</FormHelperText>
              </FormControl>
            </Grid>
            <Grid size={{ xs: 12, sm: 3 }}>
              <Controller
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, sm: 4 }}>
              <FormControl fullWidth sx={{ minWidth: 200 }}>
                ...
                <FormHelperText error={!!errors.scam_type}>{errors.scam_type?.message}</FormHelperText>
              </FormControl>
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <Controller
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth sx={{ minWidth: 200 }}>
                ...
                <FormHelperText error={!!errors.scam_approach_platform}>{errors.scam_approach_platform?.message}</FormHelperText>
              </FormControl>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth sx={{ minWidth: 200 }}>
                ...
                {scam_communication_platform === "OTHERS" && (
                  <TextField
                    ...
                    onChange={(e) => setOtherCommunicationPlatform(e.target.value)}
                    fullWidth
                    sx={{ mt: 1 }}
                    required
                  />
                )}
                <FormHelperText error={!!errors.scam_communication_platform}>{errors.scam_communication_platform?.message}</FormHelperText>
              </FormControl>
            </Grid>
            ...
            <Grid size={{ xs: 12, sm: 4 }}>
              <FormControl fullWidth sx={{ minWidth: 200 }}>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select {...field} value={field.value || ''}>
                      <MenuItem value="">Select...</MenuItem> {/* Added for reset */}
                ...
                <FormHelperText error={!!errors.scam_transaction_type}>{errors.scam_transaction_type?.message}</FormHelperText>
              </FormControl>
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <Controller
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    helperText={errors.scam_incident_description?.message || "Please describe the incident in detail, including: how you were first contacted, the sequence of events, any communications or promises made, details of transactions, timestamps, and any other relevant information that could help the investigation."}
                  />
                )}
              />
            </Grid>
...
>
  <Box sx={{ p: 2, maxWidth: '800px' }}>
    <Typography variant="subtitle1" sx={{ mb: 1 }}>Recommended Test Examples (Copy-Paste directly or use these narratives):</Typography>
    ...
    <Typography variant="body2" sx={{ mt: 1, color: "warning.main" }}>
      These align with the AI's trained scam types (Ecommerce, Phishing, Government Impersonation). Other inputs may produce suboptimal results.
    </Typography>
  </Box>
</Popover>
                  {/* <Button
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography sx={{ mt: 2 }}>
            Next steps: Our team will review the report and contact you
            if needed. For updates, contact us at general_enquiries@spf_team.com.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={warningOpen} onClose={() => setWarningOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            Using the AI assistant may automatically override your form entries based on chat suggestions. This helps create a more comprehensive report but will change what you've entered.
          </Typography>
        </DialogContent>
        <DialogActions>
