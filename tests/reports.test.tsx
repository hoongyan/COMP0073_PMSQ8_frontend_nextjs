...
const mockReports: RowType[] = [
  {
    ...
    scam_incident_description: "This is a long description that should be truncated in the table.",
    status: "Assigned",
    io_in_charge: 1,
...
describe("ScamReportList Component", () => {
  beforeEach(() => {
    jest.resetAllMocks(); // Reset mocks before each test to avoid side effects
    (reportsLib.fetchScamReports as jest.Mock).mockResolvedValue(mockReports); // Default mock for fetching reports
    (reportsLib.fetchIOs as jest.Mock).mockResolvedValue([]); // Mock IOs (empty for simplicity)
    (reportsLib.createScamReport as jest.Mock).mockResolvedValue({ ...mockReports[0], report_id: 3 }); // Mock create
    (reportsLib.updateScamReport as jest.Mock).mockResolvedValue(mockReports[0]); // Mock update
    (reportsLib.deleteScamReport as jest.Mock).mockResolvedValue(undefined); // Mock delete
    (reportsLib.fetchLinkedPersons as jest.Mock).mockResolvedValue([]); // Mock linked persons (empty)
  });

  it("renders without crashing and displays the title", () => {
    render(<ScamReportList />);
    expect(screen.getByRole("heading", { name: "Scam Reports", level: 3 })).toBeInTheDocument();
  });
  ...
  it("fetches and displays reports data in the table", async () => {
    ...
    await waitFor(() => {
      ...
      const formattedIncident1 = format(parseISO(mockReports[0].scam_incident_date), "dd/MM/yy");
      const formattedReport1 = format(parseISO(mockReports[0].scam_report_date), "dd/MM/yy");
      const formattedIncident2 = format(parseISO(mockReports[1].scam_incident_date), "dd/MM/yy");
      const formattedReport2 = format(parseISO(mockReports[1].scam_report_date), "dd/MM/yy");

      expect(screen.getByText(mockReports[0].report_id.toString())).toBeInTheDocument();
      ...
      expect(screen.getByText(mockReports[0].scam_incident_description.substring(0, 50) + "...")).toBeInTheDocument();

      expect(screen.getByText(mockReports[1].report_id.toString())).toBeInTheDocument();
  ...
  it("handles empty data", async () => {
    ...
    expect(screen.queryByText(mockReports[0].scam_type)).not.toBeInTheDocument(); 
  });
  ...
  it("opens edit dialog for a selected row", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("heading", { name: /Edit Report/i })).toBeInTheDocument(); // Adjusted to match actual title
    }, { timeout: 5000 });
  });
  ...
  it("triggers delete confirmation for a row", async () => {
    ...
    // Click confirm and check delete was called
    const confirmButton = screen.getByRole("button", { name: "Confirm" });
    await user.click(confirmButton);

    await waitFor(() => {
      expect(reportsLib.deleteScamReport).toHaveBeenCalledWith(mockReports[0].report_id);
    }, { timeout: 5000 });
